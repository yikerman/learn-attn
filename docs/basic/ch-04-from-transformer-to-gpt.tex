\chapter{From the Transformer to GPT}
...
This is the full architecture from Chapter~3. The encoder processes the source sequence with bidirectional self-attention (every token attends to every other token). The decoder generates the target sequence one token at a time with causal self-attention (each token attends only to earlier tokens), plus cross-attention layers that let the decoder read the encoder's output.
...
Keep only the decoder stack, and remove the cross-attention sublayer (since there is no encoder to attend to). Every layer is just causal self-attention followed by a feed-forward network --- exactly the transformer block from Chapter~2.
...
Recall from Chapter~3 that the original decoder layer has three sublayers:
...
That is the entire structural difference. The self-attention and feed-forward sublayers work identically to what we built in Chapter~2.

Compare the decoder-only architecture to the full encoder-decoder from Chapter~3 --- the entire left half and the cross-attention sublayer are gone:
...
\concepttag
{\small\itshape This line appears inside \code{BabyGPT.\_\_init\_\_} in \code{babygpt/model.py}.}
\begin{lstlisting}
# Sinusoidal (original paper): computed from a formula, not learned
# Learned (GPT): just another embedding table
self.wpe = nn.Embedding(context_size, n_embd)  # shape: (256, 384)
\end{lstlisting}
...
The original transformer uses ReLU in the feed-forward network. GPT replaces it with GELU, which is smooth everywhere and does not hard-zero negative inputs. This was covered in detail in Chapter~2, Section~4. Our \code{FeedForward} module uses \code{F.gelu}.

\subsection{Pre-Norm Instead of Post-Norm}

The original paper applies LayerNorm \emph{after} the residual addition (post-norm). GPT applies it \emph{before} the sublayer (pre-norm), giving the residual connection a clean identity path for gradient flow. This was covered in detail in Chapter~2, Section~3. Our \code{TransformerBlock} uses pre-norm: \code{x = x + sublayer(layer\_norm(x))}.
...
\concepttag
{\small\itshape From \code{BabyGPT.\_\_init\_\_} in \code{babygpt/model.py}.}
\begin{lstlisting}
# In BabyGPT.__init__:
...
\tip{%
\textbf{If \code{lm\_head} shares weights with \code{wte}, how does it predict the \emph{next} token?} The hidden state at position $t$ \emph{starts} as the embedding of $x_t$, but 6 transformer blocks then transform it --- attention mixes in context from positions $0 \ldots t$, and feed-forward layers apply nonlinear transformations. By the time it exits the final LayerNorm, it is no longer ``the embedding of $x_t$.'' The loss function forces the transformer blocks to push that hidden vector \textbf{toward the embedding of $x_{t+1}$}, because the tied head computes $\text{logit}[i] = \text{dot}(\text{hidden}_t,\; \text{embedding}_i)$, and the loss rewards maximizing the logit for the correct next token. Think of the embedding space as a shared room where every character has a fixed address: the transformer blocks move the hidden state from near $x_t$'s address toward $x_{t+1}$'s address, and the tied head asks ``whose address are you closest to now?'' Chapter~5 covers the loss function in detail.
}
...
The full model is in \code{babygpt/model.py}. Here is the architecture with our specific configuration from \code{babygpt/config.py}:
...
\midrule
\code{context\_size} & 256 & Maximum sequence length (context window) \\
...
\concepttag
{\small\itshape From \code{BabyGPT.forward} in \code{babygpt/model.py}.}
\begin{lstlisting}
def forward(self, tokens: torch.Tensor, targets: torch.Tensor | None = None):
    batch, seq_len = tokens.shape
    positions = torch.arange(seq_len, device=tokens.device)

    token_emb = self.transformer.wte(tokens)       # (batch, seq_len, 384)
    pos_emb = self.transformer.wpe(positions)       # (seq_len, 384) -- broadcasts over batch
    ...
    logits = self.lm_head(x)                        # (batch, seq_len, 65)
...
The input \code{tokens} has shape \code{(batch, seq\_len)} --- a batch of sequences, each \code{seq\_len} tokens long. The output \code{logits} has shape \code{(batch, seq\_len, 65)} --- a probability distribution over 65 characters at every position. When \code{targets} are provided, cross-entropy loss is computed and returned alongside the logits.

\tip{%
\textbf{Why does each position predict the \emph{next} token?} The causal mask ensures position $t$ can only attend to tokens $x_0, \ldots, x_t$ --- it cannot see $x_{t+1}$ or beyond. So the hidden vector at position $t$ is a learned summary of the past. The loss function (covered in Chapter~5) compares position $t$'s output against the target $x_{t+1}$. Gradients flow backward and reshape the model so that each position's representation becomes useful for predicting what comes next --- because that is the only thing the loss ever rewards. The architecture does not force this; training does.
}
...
\concepttag
{\small\itshape From \code{BabyGPT.\_\_init\_\_} and \code{BabyGPT.\_init\_weights} in \code{babygpt/model.py}.}
\begin{lstlisting}
...
Create \code{babygpt/model.py} with the complete implementation. Every class from the previous chapters is collected here.

\filetag{babygpt/model.py}
\begin{lstlisting}
...
from babygpt.model import BabyGPT
import torch
...
\checkpoint{You should see \texttt{Parameters: 10,745,088} and a loss near 4.17 (which is $\ln(65)$, the random baseline for 65-class classification).}
