\chapter{Generation and Sampling}
...
When the sequence grows beyond \code{context\_size} (256 in our model), we crop from the left:

\concepttag
{\small\itshape From \code{BabyGPT.generate} in \code{babygpt/model.py}.}
\begin{lstlisting}
tokens_cropped = (
...
\tip{%
\textbf{Why is it called ``temperature''?} The name comes from statistical mechanics. The Boltzmann distribution gives the probability of a physical system occupying a state with energy $E_i$ at temperature $T$: $P(\text{state}_i) \propto \exp(-E_i / kT)$. Our softmax-with-temperature has the same functional form (the sign is flipped because higher logits are \emph{better}, whereas lower energies are more favorable). The analogy maps exactly: high physical temperature means particles spread across many energy states (more disorder); high softmax temperature means probability spreads across many tokens (more randomness). As $T \to 0$, a physical system freezes into its lowest-energy ground state; our model freezes into argmax (greedy decoding). ``Turn up the temperature'' means the same thing in both worlds: more thermal energy, more randomness.
}
...
\begin{enumerate}
  ...
  \item \code{top\_values[:, [-1]]} is the smallest value among the top $k$ --- the threshold.
...
First, add the \code{generate()} method to the \code{BabyGPT} class in \code{babygpt/model.py}. This method implements the full autoregressive loop from Sections 1--4: forward pass, temperature scaling, top-k filtering, sampling, and appending.

\filetag{babygpt/model.py --- add to \code{BabyGPT} class, after \code{forward}}
\begin{lstlisting}
@torch.no_grad()
def generate(
    self,
...
Next, we need a CLI script to load a trained checkpoint, encode a prompt, call \code{model.generate()}, and print the result.

\subsection{Creating the Generation Script}

Create \code{babygpt/generate.py} --- the CLI wrapper that loads a checkpoint, encodes the prompt, calls \code{model.generate()}, and prints the result:

\filetag{babygpt/generate.py}
\begin{lstlisting}
...
uv run python -m babygpt.generate --prompt "ROMEO:" --temperature 0.8
\end{lstlisting}
...
\textbf{Temperature 0.5} (conservative): Correct spelling and formatting, but repetitive. The same phrases recur.

\textbf{Temperature 0.8} (recommended): Good balance of coherence and variety. The default in \code{babygpt/generate.py}.

\textbf{Temperature 1.2} (creative): More surprising choices, but more spelling errors and occasional gibberish.
...
This concludes the six-chapter series. The full pipeline --- from the attention mechanism to a trained, generating language model --- is implemented in \code{babygpt/}.
...
\begin{verbatim}
learn-attn-impl/
  pyproject.toml, ... uv files
  babygpt/
    __init__.py
    config.py       (Chapter~4)
    model.py        (Chapter~4)
    tokenizer.py    (Chapter~5)
    dataset.py      (Chapter~5)
    train.py        (Chapter~5)
    generate.py     (Chapter~6)
  data/
    tinyshakespeare.txt
  checkpoints/
    best.pt         (saved by babygpt/train.py)
\end{verbatim}
