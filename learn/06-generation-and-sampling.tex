...
First, add the \code{generate()} method to the \code{BabyGPT} class in \code{src/model.py}. This method implements the full autoregressive loop from Sections 1--4: forward pass, temperature scaling, top-k filtering, sampling, and appending.

\filetag{src/model.py --- add to \code{BabyGPT} class, after \code{forward}}
\begin{lstlisting}
@torch.no_grad()
...
\textbf{Temperature 0.8} (recommended): Good balance of coherence and variety. The default in \code{src/generate.py}.
...
This concludes the six-document series. The full pipeline --- from the attention mechanism to a trained, generating language model --- is implemented in \code{src/}.
