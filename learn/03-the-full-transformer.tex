\input{preamble}
\begin{document}
\docheader{The Full Transformer (Encoder-Decoder)}{3}

By now you understand the building blocks: scaled dot-product attention, multi-head attention, feed-forward networks, residual connections, and layer normalization. Documents 01 and 02 showed how these components form a transformer block. This document steps back to the original architecture from ``Attention Is All You Need'' (Vaswani et al., 2017) and explains how those blocks are arranged into a full encoder-decoder model for machine translation.
...
The critical difference from the decoder blocks in Document 02: \textbf{there is no causal mask}. Every position can attend to every other position. This is \textbf{bidirectional} attention --- position 5 can look at position 10, and position 10 can look at position 5.
...
\begin{enumerate}
  \item \textbf{Masked multi-head self-attention} (with causal mask --- same as our GPT blocks from Document 02)
...
Cross-attention uses the exact same multi-head attention mechanism from Document 01, but the queries, keys, and values come from different places:
...
\tip{%
\textbf{What is BPE?} Byte Pair Encoding is a subword tokenization algorithm. Start with a vocabulary of individual characters. Count every adjacent pair of tokens in the training corpus, merge the most frequent pair into a new token, and repeat for a fixed number of merges (which controls the final vocabulary size). Common words like ``the'' become single tokens; rare words like ``unhappiness'' get split into subword pieces (e.g.\ ``un'' + ``happiness''). This gives a compact vocabulary with no out-of-vocabulary problem --- any string can be represented. GPT-2 uses byte-level BPE, where the base vocabulary is all 256 bytes. Our BabyGPT sidesteps tokenization entirely by treating each character as its own token (vocab size 65).
} Sharing means the model uses one set of embedding vectors for, say, the token ``the'' regardless of whether it appears on the source side or the target side.
...
\textbf{Why sinusoidal and not learned?} The paper argues that sinusoidal encodings may generalize to sequence lengths longer than those seen during training, since the sinusoidal functions are defined for any position. In practice, learned positional embeddings (as used in GPT-1 and GPT-2) work just as well for fixed-length contexts and are simpler to implement. Our BabyGPT in Document 04 will use learned positional embeddings.
...
\begin{itemize}
  ...
  \item \textbf{Training time:} \textasciitilde{}3.5 days for the base model, \textasciitilde{}12 hours per training step on 8 GPUs
...
GPT (Radford et al., 2018) simplifies the transformer by keeping only the decoder (with its causal mask), removing the encoder and cross-attention entirely. What remains is a stack of transformer blocks, each containing masked self-attention and a feed-forward network --- exactly the blocks we built in Document 02.

Compare the decoder-only architecture to the full encoder-decoder above --- the entire left half and the cross-attention sublayer are gone:
...
% Input label
\node[lbl] (input) at (\cx, 0) {Input Tokens};

% Token Embedding
\node[emb] (emb) at (\cx, 0.8) {Token Embedding};
\draw[arr] (input) -- (emb);

% Positional embedding
\node[lbl] (pe) at (\cx, 1.55) {$\oplus$ Positional Embedding};
\draw[arr] (emb) -- (pe);

% N× block
\begin{scope}[on background layer]
  \draw[rounded corners=5pt, fill=gray!12, draw=gray!50]
    (\cx-1.75, 2.0) rectangle (\cx+1.75, 5.0);
\end{scope}
\node[lbl, gray] at (\cx+1.45, 2.2) {$\times N$};

% Masked self-attention
\node[attn] (msa) at (\cx, 2.7) {\shortstack{Masked\\[-1pt]Multi-Head Attention}};
\draw[arr] (pe) -- (msa);

\node[norm] (n1) at (\cx, 3.45) {Add \& Norm};
\draw[arr] (msa) -- (n1);
\draw[res] ([xshift=-1.6cm]msa.south) |- (n1.west);

% Feed forward
\node[ffn] (ff) at (\cx, 4.1) {Feed Forward};
\draw[arr] (n1) -- (ff);

\node[norm] (n2) at (\cx, 4.65) {Add \& Norm};
\draw[arr] (ff) -- (n2);
\draw[res] ([xshift=-1.6cm]ff.south) |- (n2.west);

% Output head
\node[head] (lin) at (\cx, 5.5) {Linear (tied weights)};
\draw[arr] (n2) -- (lin);

\node[head] (soft) at (\cx, 6.15) {Softmax};
\draw[arr] (lin) -- (soft);

\node[lbl] (oprob) at (\cx, 6.8) {Next-Token Probabilities};
\draw[arr] (soft) -- (oprob);

\end{tikzpicture}
\end{center}

In Document 04, we will make this simplification explicit and assemble our BabyGPT.
